...
void suaSinhVien() {
    ...
    if (file != NULL) {
        ...
        while (fread(&sv, sizeof(SinhVien), 1, file) == 1) {
            if (strcmp(sv.maSV, maSV) == 0) // xét đkien nếu string compare maSV (giá trị nhập vào) và sv.maSV trả về true( giống nhau)
            {
...
void xoaSinhVien() {
    ...
    if (file != NULL && temp != NULL) {
        ...
        if (found == 1) {
            printf("Da xoa du lieu cua sinh vien co ma %s\n", maSV);
        } else {
            printf("Khong co thong tin cua sinh vien can xoa\n");
        }
    } else {
        printf("Khong the mo file student.txt\n");
    }
...
void timKiemTheoDiemTrongKhoang() {
    float min, max;
    do {
        printf("Nhap khoang diem TB can tim kiem (min-max)\n");
        printf("Nhap min: ");
        scanf("%f", &min);
        printf("Nhap max: ");
        scanf("%f", &max);
        printf("Khoang diem TB can tim kiem (%.2f-%.2f) \n",min,max);
        if (min < 0 || max > 10) {
